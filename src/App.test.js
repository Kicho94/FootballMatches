...
test("search is made on by clicking on search button and no results found", async () => {
  ...
  const url =
    "https://jsonmock.hackerrank.com/api/football_competitions?year=2017";
  fetchMock.getOnce(
    url,
    JSON.stringify({
      page: 1,
...
test("search is made on by clicking on search button and result found - test 1", async () => {
  ...
  const url =
    "https://jsonmock.hackerrank.com/api/football_competitions?year=2011";
  fetchMock.getOnce(
    url,
    JSON.stringify({
      page: 1,
  ...
  await wait(() => {
    expect(queryByTestId("total-matches")).toHaveTextContent(
      "Total matches: 3"
    );
    const results = queryByTestId("match-list");
    expect(results.childNodes).toHaveLength(3);
    expect(results.childNodes[0]).toHaveTextContent(
      "Match UEFA Champions League won by Chelsea"
    );
    expect(results.childNodes[1]).toHaveTextContent(
      "Match Serie A won by Juventus"
    );
    expect(results.childNodes[2]).toHaveTextContent(
      "Match Bundesliga won by Borussia Dortmund"
    );
    expect(queryByTestId("no-result")).toBe(null);
  });
});
...
test("search is made on by clicking on search button and result found - test 2", async () => {
  ...
  const url =
    "https://jsonmock.hackerrank.com/api/football_competitions?year=2013";
  fetchMock.getOnce(
    url,
    JSON.stringify({
      page: 1,
  ...
  await wait(() => {
    ...
    expect(results.childNodes[0]).toHaveTextContent(
      "Match English Premier League won by Manchester City"
    );
    expect(results.childNodes[1]).toHaveTextContent(
      "Match La Liga won by Atletico Madrid"
    );
    expect(queryByTestId("no-result")).toBe(null);
  });
});
